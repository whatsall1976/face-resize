...
def make_anchor_points(oval_points, center, x_axis, y_axis, width, height, anchor_scale, samples_per_side=9):
    ...
    half_w = (max_x - min_x) * 0.5 * anchor_scale
    half_h = (max_y - min_y) * 0.5 * anchor_scale

    left = box_center_x - half_w
    right = box_center_x + half_w
    top = box_center_y - half_h
    bottom = box_center_y + half_h
...
def build_control_points(landmarks, width, height, scale_x, scale_y, anchor_scale):
...
def resize_face_local(input_path, output_path, scale_x, scale_y, anchor_scale, debug_points=None, debug_mesh=None):
